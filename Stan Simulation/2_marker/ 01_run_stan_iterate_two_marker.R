...
source("Estimation/process_mb.R")
...
MOD_NAME <- "Multi_marker_amp_fix_v9.stan"
THEME = paste0("V9.fin_3_rep_amp-sd=",AMP_SD,"_")
NOM <- paste0(THEME,ind_sim,"_TWO_sim_2025-11-30_",n_site,"site_",N_sp,"sp_conc.Rdata")

sim_scen <- data.frame(N_SP = rep(N_sp,n_site),
                           TOT.CONC = TOT.CONC,
                           CONC.DIST = rep("very_high_skew",n_site),
                           N_READS = rep(100000,n_site),
                           amp_sd = rep(AMP_SD,n_site),
...
for(j in 1:ind_sim){# loop over number of independent sim
  ...
  for( i in 1:nrow(sim_scen)){
    source(here("Zenodo","Stan Simulation","1_marker","simulate_2marker_metabar_iterate.R"))
    OUT[[i]] <- process_mb2(out,site_name=paste0("X",i))
  }
  source(here("Zenodo","Stan Simulation","1_marker","02M_process_sim_iterate_two_marker.R"))
  source(here("Zenodo","Stan Simulation","1_marker","03M_estimate_stan_iterate_two_marker.R"))
  source(here("Zenodo","Stan Simulation","1_marker","04M_post_process_stan_iterate_two_marker.R"))
}


# Pull out any replicates that had divergent transitions and throw them away.
THESE <- divergences %>% as.data.frame() %>% filter(count> 10) %>% pull(rep_id)
...
post_tot_conc_all <- post_tot_conc_all %>% mutate(prop_error = (Mean - tot_conc_true) / tot_conc_true)
conc_dat_all <- conc_dat_all %>% mutate(prop_error = (Mean - conc_val) / conc_val)
...
p_prop_error_tot_conc_rep <- ggplot(post_tot_conc_all) +
  ...
  scale_x_continuous(trans="log", "True total conc.",breaks=BRKS) +
  scale_y_continuous("Proportional Error") +
  #coord_cartesian(xlim=c(min(BRKS),max(BRKS)),ylim=c(min(BRKS),max(BRKS))) +
  geom_hline(yintercept =0, linetype="dashed", color="red")+
  theme_bw()
...
OUT <- list(stan_data_list = stan_data_list,
            ...
            plots=plots)
save(OUT,file=here("data","Model_est_2marker_sim",NOM))
